...
function App() {
  ...
  const handleDeleteImageInUrlPreview = () => {
    if (urlPreview.length === 1) {
      ...
      const numberFind = urlPreview.findIndex((url) => url.src == defaultImg[0].src)
  ...
  const fetAllListEachBranch = async(number) => {
        ...
        for ( let i = 0; i < typeList.length; i++) {
          const listRef = ref(storage, `${branchList[number].name}/${typeList[i]}`);
            await listAll(listRef).then((res) => {totalImagesInEachBranch += res.items.length}).catch((error) => {
                console.log(error.message, "error getting the images url");
              });
        }
        ...
        const fetOneBranch = async () => {
          ...
          while (a < typeList.length - 1) {
            a++
            const listRef = ref(storage, `${branchList[number].name}/${typeList[a]}`);
            await listAll(listRef)
             .then((res) => {
               res.items.map(async(itemRef) => {
                  getDownloadURL(itemRef)
  ...
  const handleDelete = (itemRef) => {
    deleteObject(itemRef).then(() => {
      if (branch === "Tất cả") {setIsDeleteInAllBranch(!isDeleteInAllBranch); setNumberBranchAction(numberBranch)}
  ...
  return (
    <div style={{height: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center"}}>
      <AlertMsg />
      {branch && type ? "" : <p style={{color: "red", fontSize: "18px"}}>VUI LÒNG CHỌN BRANCH VÀ TYPE TRƯỚC KHI TẢI ẢNH LÊN</p>}
                        ...
                        <div style={{ width: "80vw", height: branch.list.length > 0 ? "300px" : "50px", overflow: "auto", display: "flex", flexWrap: "wrap", padding: "20px", backgroundColor: "#EEEEEE"}}> 
                          ...
                          return (
                            <div style={{ width: "250px", height: "250px", overflow: "hidden", position: "relative"}}>
                              <img onClick={() => { setVisible(true); setDefaultImg([{"src": element.src}]); setImageSelected(element.itemRef); setNumberBranch(index) } } width="250px" height="250px" src={element.src} alt="not found" ></img>              
                            </div>
                          )
                        }
                          )
      ...
      >
        <Box sx={style}>
          ...
          <div style={{marginTop: "20px"}}>
            ...
            <Button variant='outlined' style={{backgroundColor: "red", color: "white"}} onClick={() => {handleClose(); urlPreview.length > 0 ? handleDeleteImageInUrlPreview() : handleDelete(imageSelected)}}>Delete</Button>
          </div>
        </Box>
      </Modal>
      ...
      <div style={{ width: "80vw", textAlign: "end", display: "flex", justifyContent: "space-between"}}>
        ...
        <Checkbox
          checked={checked}
          onChange={urlPreview.length > 0 ? handleCheckBoxUrlPreview : handleChangeCheckBox}
          inputProps={{ 'aria-label': 'controlled' }}
        />
        <p>{urlPreview.length > 0? "Xóa tất cả ảnh sắp tải lên" : "Chọn tất cả ảnh"}</p>
        </div>
        }
        {typeAll ? <div></div> : 
          <p>{ urlPreview.length > 0 ? ( `Có ${urlPreview.length} ảnh sẵn sàng tải lên` ): ("")}</p>
        }
        {
      ...
      <div style={{ display: "flex", alignItems: "center"}}>
        ...
        checked ? (
          <LoadingButton style={{ width: "auto", height: "50px"}} onClick={urlPreview.length > 0 ? handleDeleteAllUrlPreview : handleDeleteAll} loading={isLoadingDelete} variant="contained">
           Delete All
          </LoadingButton>
        ) :
